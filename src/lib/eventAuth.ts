...
export async function GET(req: NextRequest) {
  ...
  // This route is only for the organizer's management dashboard (unlike
  // s3-upload-url, which intentionally stays open for guest uploads via the
  // public event link) — so it must confirm the caller owns the event.
